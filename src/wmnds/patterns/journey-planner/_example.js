...
const journeyPlannerJS = () => {
  ...
  const expandComponent = e => {
    e.target.closest('.wmnds-journey-planner').classList.toggle(openClass);
    e.target.closest('.wmnds-journey-planner__submit').classList.toggle('wmnds-m-t-lg');
  ...
  const disableAllOptionsBeforeNow = firstAvailableSlot => {
    let timeSlots;
    if (currentComponent.classList.contains('wmnds-journey-planner__when')) {
      ...
      timeSlots = currentComponent
        .closest('.wmnds-journey-planner')
        .querySelectorAll(
          ".wmnds-journey-planner__when .wmnds-fe-dropdown__select[name='SelectedJourneyTime'] option"
        );
    }
  ...
  const updateTimeOptions = (component = currentComponent) => {
    const day = component.querySelector(".wmnds-fe-dropdown__select[name='day']").value;
    if (day === 'today') {
      ...
      let slots;
      if (component.classList.contains('wmnds-journey-planner__when')) {
        slots = component.querySelectorAll(
          ".wmnds-fe-dropdown__select[name='SelectedJourneyTime'] option"
        );
      } else {
        slots = component
          .closest('.wmnds-journey-planner__when')
          .querySelectorAll(".wmnds-fe-dropdown__select[name='SelectedJourneyTime'] option");
      }
  ...
  const init = () => {
    ...
    const switchBtn = currentComponent.querySelector('.wmnds-journey-planner__switch');
    switchBtn.addEventListener('click', switchFromAndTo);

    // Update time options based on the day selected and current datetime
    updateTimeOptions(currentComponent.querySelector('.wmnds-journey-planner__when'));

    // Add event listener to travel day
    const day = currentComponent.querySelector(
      '.wmnds-journey-planner__when .wmnds-fe-dropdown__select[name="day"]'
    );
    ...
    const addDetailsBtn = currentComponent.querySelector(
      '.wmnds-journey-planner__submit button.wmnds-btn'
    );
    addDetailsBtn.addEventListener('click', expandComponent);

    // Add event listener to submit button
    currentComponent
      .querySelector('.wmnds-journey-planner__form')
      .addEventListener('submit', validateBeforeSubmit);
  };

  const components = document.querySelectorAll('.wmnds-journey-planner');
