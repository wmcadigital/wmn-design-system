const findATimetableJS = () => {
  const findATimetableComponents = document.querySelectorAll('.tfwmds-find-a-timetable-widget');
  const toggleOpenClass = e => {
    e.target.closest('.tfwmds-find-a-timetable-widget').classList.toggle('tfwmds-is--open');
  };
  ...
  findATimetableComponents.forEach(findATimetableComponent => {
    findATimetableComponent
      .querySelector('div.tfwmds-btn.tfwmds-btn--mode')
      .addEventListener('click', toggleOpenClass);
    findATimetableComponent
      .querySelector('.tfwmds-find-a-timetable-widget__form')
