const headerJs = () => {
  ...
  const megaMenus = document.querySelectorAll('.tfwmds-mega-menu');
  ...
  */
  ...
  megaMenus.forEach(menu => {
    const clearActiveListItems = () => {
      ...
      menu.querySelectorAll('.tfwmds-mega-menu__primary-menu-item').forEach(menuItem => {
    ...
    function handleMobileMenu(mq) {
      if (mq.matches) {
        const mobileToggle = menu.querySelector('.tfwmds-mega-menu__mobile-toggle');
        const headerEl = menu.parentNode.parentNode;

        const topLevelMenuBtn = menu.querySelectorAll('.tfwmds-mega-menu__link-arrow-icon-btn');
        const searchBtn = menu.querySelector('.tfwmds-mega-menu__search-btn');
        ...
        mobileToggle.addEventListener('click', () => {
          ...
          if (mobileMenuIsOpen.menu) {
            ...
            headerEl.classList.remove('tfwmds-header--search-open');
            headerEl.classList.add('tfwmds-header--mega-menu-open');
            document.querySelector('html').classList.add('mobile-menu-open');
          } else {
            headerEl.classList.remove(
              'tfwmds-header--mega-menu-open',
              'tfwmds-header--mega-menu-submenu-open'
            );
        ...
        if (searchBtn) {
          searchBtn.addEventListener('click', () => {
            ...
            if (mobileMenuIsOpen.search) {
              ...
              document.querySelector('html').classList.remove('mobile-menu-open');
              headerEl.classList.add('tfwmds-header--search-open');
            } else {
              headerEl.classList.remove('tfwmds-header--search-open');
            }
          });
        ...
        topLevelMenuBtn.forEach(menuBtn => {
          const handleSubMenus = () => {
            ...
            if (mobileMenuIsOpen.primary) {
              ...
              targetListItem.querySelector('.tfwmds-mega-menu__sub-menu-link').focus();
              headerEl.classList.add('tfwmds-header--mega-menu-submenu-open');
            } else {
              targetListItem.classList.remove('open');
              headerEl.classList.remove('tfwmds-header--mega-menu-submenu-open');
            }
          };
        ...
        const collapseMenus = menu.querySelectorAll(
          '.tfwmds-mega-menu__sub-menu-item .tfwmds-mega-menu__collapse-toggle'
        );
    ...
    const topLevelLinks = menu.querySelectorAll('.tfwmds-mega-menu__primary-menu-link');
    let menuDelay = false;
    let enterTimeOut;
    let leaveTimeOut;
    const delayTime = 300;
    ...
    topLevelLinks.forEach((topLevelLink, topLevelLinkIndex) => {
      ...
      const topLevelListItem =
        ...
        topLevelLink.parentNode.className.includes('tfwmds-mega-menu__search')
          ? topLevelLink.parentNode
          : topLevelLink;

      const subMenuLinks = topLevelListItem.querySelectorAll('.tfwmds-mega-menu__sub-menu-link');

      // check if level 3 menus are present, if so add modifier class
      const hasSubmenuChildren =
        topLevelListItem.querySelectorAll('.tfwmds-mega-menu__sub-menu-child-menu').length !== 0;
      if (hasSubmenuChildren) {
        topLevelListItem.querySelectorAll('.tfwmds-mega-menu__sub-menu').forEach(subMenu => {
          subMenu.classList.add('tfwmds-mega-menu__sub-menu--has-child-menus');
        });
      }

      const openSubMenu = e => {
        // check if list item has a mega menu
        if (topLevelListItem.querySelectorAll('.tfwmds-mega-menu__container').length) {
          e.preventDefault();
          ...
          } else if (topLevelListItem.querySelector('.tfwmds-search-bar__input')) {
            topLevelListItem.querySelector('.tfwmds-search-bar__input').focus();
          }
        }
      };
      ...
      const isTopLevelWithMenu = topLevelListItem.querySelectorAll('.tfwmds-mega-menu__container').length;
      ...
      if (isTopLevelWithMenu) {
        ...
        topLevelListItem
          .querySelector('.tfwmds-mega-menu__container')
      ...
      const subMenuContainer = topLevelListItem.querySelector('.tfwmds-mega-menu__sub-menu');

      if (subMenuContainer) {
        setKeyboardNavigation(
          subMenuContainer,
          '.tfwmds-mega-menu__sub-menu-item',
    ...
    const searchMenuContainer = menu.querySelector('.tfwmds-search-container');
    if (searchMenuContainer) {
      setKeyboardNavigation(
        searchMenuContainer,
        '.tfwmds-search-list',
        // what to do on first link
        () => menu.querySelector('.tfwmds-search-bar__input').focus(),
        // what to do on last link
        () =>
          setMenuActive(
            menu.querySelector('.tfwmds-mega-menu__search'),
            false,
            menu.querySelector('.tfwmds-mega-menu__search-btn')
          )
      );
    }
  });
