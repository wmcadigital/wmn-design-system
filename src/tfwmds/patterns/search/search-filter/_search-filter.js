const searchFilterJs = () => {
  ...
  if (searchFilter) {
    const filterOptions = searchFilter.querySelectorAll('.tfwmds-fe-checkboxes__input');

    filterOptions.forEach(option => {
      option.addEventListener('change', () => {
        if ([...filterOptions].some(input => input.checked)) {
          searchFilter.classList.add('tfwmds-search-filter--has-inputs-checked');
        } else {
          searchFilter.classList.remove('tfwmds-search-filter--has-inputs-checked');
        }
      });
    });
    ...
    const showFilter = (show = true) => {
      if (show) {
        searchFilter.classList.add('tfwmds-search-filter--is-open');
        searchFilter.setAttribute('aria-expanded', 'true');
      } else {
        searchFilter.classList.remove('tfwmds-search-filter--is-open');
    ...
    document.querySelectorAll('.tfwmds-search-filter__clear-all').forEach(clearBtn => {
