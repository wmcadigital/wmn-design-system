...
const nIcons = () => {
  ...
  const convertStringToNode = string => {
    ...
    const svgIcon = `
      <svg class="tfwmds-n-icon__svg">
        <title>N-Network icon</title>
        <desc>A hexagon with the letter 'n' inside of it.</desc>
        <use xlink:href="#tfwmds-general-n-ticket" href="#tfwmds-general-n-ticket"></use>
      </svg>`;
    // Then replace our found text with 'n' icon and text without 'n'
    const htmlString = `<span class="tfwmds-n-icon">${svgIcon}${textWithoutN}</span>`;
