const tableJS = () => {
  const tables = document.querySelectorAll('.tfwmds-table');
  ...
  const iterateTables = table => {
    ...
    if (!theadChildren.length) return table.classList.add('tfwmds-table--without-header'); // If no headers found, add without-header class
